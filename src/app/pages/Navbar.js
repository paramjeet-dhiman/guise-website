import { useEffect, useRef, useState } from 'react';
import { NavLink } from 'react-router-dom'
import LOGO from "../../assets/images/GuiseLOGO.png";


const MenuItems = [
    {
        label: 'Home',
        url: "/",
        active: true,
        icon: <svg className="text-blue-500 w-6 h-6" fill="currentColor" viewBox="0 0 20 20" ><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path></svg>
    },
    {
        label: 'Solutions',
        url: "/solutions",
        active: false,
        icon: <svg className="text-blue-500 w-6 h-6" fill="currentColor" viewBox="0 0 20 20" ><path d="M10 3.5a1.5 1.5 0 013 0V4a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-.5a1.5 1.5 0 000 3h.5a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-.5a1.5 1.5 0 00-3 0v.5a1 1 0 01-1 1H6a1 1 0 01-1-1v-3a1 1 0 00-1-1h-.5a1.5 1.5 0 010-3H4a1 1 0 001-1V6a1 1 0 011-1h3a1 1 0 001-1v-.5z"></path></svg>
    },
    {
        label: 'Contact',
        url: "/contact",
        active: false,
        icon: <svg className="text-blue-500 w-6 h-6" fill="currentColor" viewBox="0 0 20 20" ><path fillRule="evenodd" d="M7 2a2 2 0 00-2 2v12a2 2 0 002 2h6a2 2 0 002-2V4a2 2 0 00-2-2H7zm3 14a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd"></path></svg>
    },

];



export const Navbar = () => {
    const [isOpen, setIsOpen] = useState(false);
    const menuRef = useRef();

    useEffect(() => {
        let handler = (e) => {
            if (!menuRef.current.contains(e.target))
                setIsOpen(false)
        };
        document.addEventListener("mousedown", handler);
        return () => {
            document.removeEventListener("mousedown", handler);
        }
    }, [])


    return (
        <nav className={` bg-gray-900 shadow-xl  fixed inset-x-0 z-10 backdrop-filter backdrop-blur-lg bg-opacity-40`} ref={menuRef}>
            <div className="mx-auto text-white">
                <div className="flex items-center justify-between h-16 px-5  md:px-10 ">
                    <NavLink to="/" className="flex-shrink-0 flex items-center">
                        <svg height="30" viewBox="0 0 4421 724" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                            <rect width="4421" height="724" fill="url(#pattern0)" />
                            <defs>
                                <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                    <use xlinkHref="#image0" transform="scale(0.000693963 0.00423758)" />
                                </pattern>
                                <image id="image0" width="1441" height="236" xlinkHref="data:image/png;base64," />
                            </defs>
                        </svg>
                    </NavLink>

                    <div className=" space-x-4 hidden md:flex">
                        {
                            MenuItems.map((item, index) => (
                                <NavLink
                                    exact
                                    to={item.url}
                                    key={index}
                                    activeClassName="text-white border-b-4 border-blue-500 font-black  cursor-pointer"
                                    className={` text-gray-200  text-base px-3 py-2  hover:text-blue-500  font-semibold border-b-4  border-transparent hover:border-current cursor-pointer select-none`}
                                    aria-current="page"
                                >
                                    {item.label}
                                </NavLink>
                            ))
                        }

                    </div>


                    <div className="-mr-2 -my-2 md:hidden">
                        <button
                            type="button"
                            className="bg-gray-800 inline-flex items-center justify-center p-2 rounded-md text-gray-100 hover:text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white"
                            onClick={() => setIsOpen(!isOpen)}
                        >
                            <span className="sr-only">Open menu</span>
                            <svg
                                className="h-6 w-6"
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                aria-hidden="true"
                            >
                                <path
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth={2}
                                    d="M4 6h16M4 12h16M4 18h16"
                                />
                            </svg>
                        </button>
                    </div>

                    <div
                        className={
                            isOpen
                                ? "visible scale-100 transition ease-out duration-200 absolute top-0 inset-x-0 p-2  transform origin-top-right md:hidden"
                                : "invisible scale-95 absolute top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden"
                        }
                    >
                        <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50">
                            <div className="pt-5 pb-6 px-5">
                                <div className="flex items-center justify-between">
                                    <img
                                        className="h-8 w-auto sm:h-10"
                                        src={LOGO}
                                        alt="logo"
                                    />
                                    <div className="-mr-2">
                                        <button
                                            type="button"
                                            className="bg-gray-900 inline-flex items-center justify-center p-2 rounded-md text-gray-50 hover:text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white"
                                            onClick={() => setIsOpen(!isOpen)}
                                        >
                                            <span className="sr-only">Close menu</span>

                                            <svg
                                                className="h-6 w-6"
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                stroke="currentColor"
                                                aria-hidden="true"
                                            >
                                                <path
                                                    strokeLinecap="round"
                                                    strokeLinejoin="round"
                                                    strokeWidth={2}
                                                    d="M6 18L18 6M6 6l12 12"
                                                />
                                            </svg>
                                        </button>
                                    </div>
                                </div>
                                <div className="mt-6">
                                    <nav className="grid gap-y-8">

                                        {
                                            MenuItems.map((item, index) => (
                                                <NavLink
                                                    exact
                                                    to={item.url}
                                                    key={index}
                                                    activeClassName="text-gray-800 cursor-pointer bg-gray-100"
                                                    className="-m-3 p-3 flex text-gray-800 items-start rounded-lg  hover:bg-gray-200"
                                                    aria-current="page"
                                                >
                                                    <span className="ml-3 text-base flex justify-center items-center space-x-1 font-semibold ">
                                                        <span>
                                                            {item.icon}
                                                        </span>
                                                        <span>
                                                            {item.label}
                                                        </span>

                                                    </span>
                                                </NavLink>
                                            ))
                                        }
                                    </nav>
                                </div>
                            </div>
                        </div>
                    </div>




                </div>
            </div>
        </nav >
    )
}


